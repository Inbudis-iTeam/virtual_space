...
@section('title') Catégories d'utilisateurs @stop
    ...
    <div class="content">
        <div class="container-fluid">
            <div class="col-md-12">
                <div class="card">
                    <div class="header">
                        <h4 class="title">Catégories d'utilisateurs</h4>
                        <p class="category">
                    ...
                    <div class="content table-responsive table-full-width">
                        <table class="table table-hover table-striped">
                            ...
                            <tbody>
                                <?php $i = 0?>
                                @forelse ($categoriesUser as $categorieUser)
                                <?php $i++?>
                                <tr>
                                    <td>{{$i}}</td>
                                    <td>{{$categorieUser['titre_catuser']}}</td>
                                    <td class="dropdown">
                                        ...
                                        <ul class="dropdown-menu">
                                            <li><a href="{{url('categories_user/modify') ."/". $categorieUser['id']}}#addModal" class="modify_item" data-toggle="modal" data-titre="{{$categorieUser['titre_catuser']}}">Modifier</a></li>
                                            <li>
                                                <a href="{{url('categories_user/delete')."/". $categorieUser['id']}}" class="delete_item">Supprimer</a>
                                            </li>
                                        </ul>
                                    </td>
                                </tr>
                                @empty
                                    <tr><td colspan="3" class="text-center">Aucune catégorie enregistrée. Ajoutez s'en une dès maintenant <br><a href="#addModal" data-toggle="modal" class="btn btn-info add_item">Ajouter</a></td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
...
<div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="basicModal" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form method="post" action="{{url('/categories_user/add')}}" id="addForm">
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $('.add_item').on('click', function (e) {
                ...
                $("#message").html('');
                $('#addForm').attr("action", "{{url('categories_user/add')}}");
                $("#nameItem").val('');
            });
